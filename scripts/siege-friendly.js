  ...
  while (true) {
    ...
    try {
      ...
      if (isAsteroidPlanet(targetCell)) {
        blockedTargets.set(targetKey, Date.now() + 60 * 60 * 1000);
        currentTargetKey = null;
        lastTargetLogKey = null;
        printStatus(
          `${getTargetLabel(targetCell)} ignoree: champ d'asteroides non capturable`
        );
        await sleep(500);
        continue;
      }
      ...
      if (hitPoints <= targetHp) {
        ...
        const claimTargetLookup = await getLocalCellLookup([claimShip, target], 3);
      ...
      for (const ship of attackOrder) {
        ...
        if (isAdjacent(ship, target)) {
          ...
          if (sidestepCell) {
            ...
            try {
              ...
              printStatus(
                `${ship.nom} replannifie: la case (${sidestepCell.x}, ${sidestepCell.y}) est deja occupee`
              );
              continue;
            }
            actionsDone += 1;
        ...
        const shipLocalLookup = await getLocalCellLookup([ship, target], 3);
        const moveTarget = chooseNextStep(
          ship,
          stage,
          occupiedCells,
